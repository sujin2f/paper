...
특정 파장의 광자를 흡수한 전자는 그 궤도가 n이라는 정수만큼 증가하며, 해당 파장의 광자를 방출할 때 궤도는 감소한다. 여기서 에테르 독립적 스파클인 광자가 $E=m {c^{2}}$에 의하여 스파클 독립적 에테르로 변화하여 궤도 사이에 들어가는 것으로 생각의 틀을 바꿔보도록 하자. $n \geqq 2$
...
광자라는 스파클은 해당하는 에너지 만큼의 에테르-질량으로 전환되어 원자핵과 전자 사이에 위치하게 된다. 반대로 에너지를 방출할 때에는 에테르가 스파클로 전환되어 광자의 형태로 방출되는 것이라 가정해 보도록 하자.  $n \geqq 2$
...
수소 원자 모형에서는 괄호 속의 첫번째 항을 $\frac{1}{4}$의 형태로 만들었지만, 여기서는 두번째 항으로 해 보겠다. $(\frac{\epsilon}{2})^2 = \frac{2\mu}{ℏ^2}V(r)$로, 그리고 $x=r\epsilon$로 치환하여 식2-6을 아래와 같이 만든다.
