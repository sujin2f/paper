...
가설에 따르면 전자는 그저 존재할 뿐이며 빛 에테르는 전자와 원자핵 사이의 공간에 들어오는 것이다. 오비탈에서 검은 부분을 마디(node)라 하는데, 이 부분이 빛의 에테르일 것이다. 주 양자수 n에 따라 원형 에테르의 개수가 증가하고 부양자수 $l$에 따라 원형의 에테르 하나가 선형으로 변화하게 된다. 빛 에테르 분포가 전자 오비탈의 마디와 일치한다면 오비탈의 마디는 사실 빛 에테르라는 것을 검증할 수 있을 것이다. 확률 분포는 슈뢰딩거 방정식으로 알아볼 수 있다.
...
$$R''(r) + \frac{2}{r} R'(r) + \frac{2 \frac{E_n}{c^2}}{ℏ^2} (-\frac{m_e {e^{4}}}{32 \pi ^ 2 \epsilon_{0}^2 ℏ^2} (\frac{1}{(n-1)^2} - \frac{1}{n^2}-mr (\frac{E}{ℏ})^2 \frac{1}{((\dfrac{1}{(n-1)^2} - \dfrac{1}{n^2}) \times 2n)^2})R(r) + \frac{l(l+1)}{r^2}R(r) = 0$$
