...
이것을 빛-에테르로 표기해 보겠다. 에테르는 원형과 선형이 존재한다. 원형은 `O`로, 선형은 `-`로 표기하며 채워지는 순서대로 `[]`속에 채운다. 이 표기법에 따라 `3s`는 원형 에테르 두 개를 의미하기에 `[OO]`로 표시할 수 있다. 선형 에테르 하나와 원형 에테르 하나로 이루어진 `2p`의 표기법은 `[-O]`가 되겠다. 에테르가 존재하지 않는다면 `[X]`로 표기한다. 방출 파장은 뤼드베르 공식을 따르겠다. 여기서 `Nth`는 에너지의 크기를 결정하는 부분이기 때문에 별도로 에너지 계수라고 부르도록 하겠다.

$$\lambda = \frac{1}{R(N_{th})}10^{-4}$$
...
잘 일치하지만 이건 매우 당연한 결과이다. 고전역학의 뤼드베리 공식이 수소원자에서 잘 맞는다는 것을 보여주는 것이기 때문이다. 수소에서는 에테르가 원형인지 선형인지와는 상관없이 값이 잘 맞는다. 여기서는 에테르의 표기법과 에너지 계수라는 개념만을 가져가도록 한다.
