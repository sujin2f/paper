...
우리는 `L`항의 `n-l-1` 부분이 항상 `1`이 되는 것으로 가정해서 제거했다. 따라서 오차의 보정은 `x^l`항에서 일어나게 될 것이다. 그렇다면 자연스럽게 `l`이 의미하는 바가 새롭게 정의될 것이다. `x`의 `l`승에서 `l`은 `(k+1)/2`에서 `l+1`로 변화했으며 `y(r) = rR(r)`로 치완하는 과정에서 `l`로 변화했다. 따라서 이는 `(k-1)/2`로 표기해도 무방하다. 위에서 정의한 `Nn`까지 적용해서 `Rth`를 다시 써보기로 하겠다.

$$x = \frac{2r}{a_0} \sqrt{N_{th}(n_{th}) \frac{1}{n_{th}}}$$

$$R_{th}(r, n_{th}) = Ae^{-\frac{x}{2}} x^{\frac{k-1}{2}} L_{j}^{k}(x)$$
...
우리는 앞에서 `(2j + k + 1)/2`를 `n - 1`으로 치환했다. 그런데 다시 수소원자로 돌아가보면, 이 과정에서의 `n - 1`은 원래 `n`이었으며, 이는 `x`에서 `n^2`으로 위치하게 된다. 그런데 식3-4에서 `E = -1/(n-1) * ɑ`라 했는데, `E`는 `Nn`역시 포함하고 있기 때문에 `E = -Nn/(n-1) * ɑ`로도 쓸 수 있다. 앞서 밝힌대로 숫자 놀음을 해보기로 하자. 위에서 `n - 1`이 `n - l - 1`로 변화한 것 까지를 포함해 `(2j + k + 1)/2`의 제곱을 `Nn/(n-1)`으로 치환하는 것이다.

$$(\frac{2j + k + 1}{2})^2 = Nn/(n-1)$$

$$\to \frac{2j + k + 1}{2} = \sqrt{Nn/(n-1)}$$

$$\to 2j + k + 1 = 2\sqrt{Nn/(n-1)}$$

$$\to k = 2\sqrt{Nn/(n-1)} - (1 + 2j)$$

이제 `j`를 다시 재정의 해보자. 식3-4를 다시 보면 그것이 원래는 `E = -x/2(n-1) * V(r)`이었다는 것을 알 수 있다. 그리고 매우 억지스럽지만 수소 원자에서 `n`이 `n^2`이 되는 것과 같이 이번에는 오른쪽 항을 `1/2(n -1)^2`로 만드는 것이다. 그리고 `k`는 `L`의 윗 인자이기도 하다. `L`의 아래 인자를 `1`로 가정한 것과 같이 여기서는 `k`를 항상 `0`이라 하자. 알고있다. 매우 억지스럽다는 것을.

$$\frac{2j + k + 1}{2} = \frac{1}{2(n-1)^2}$$

$$\to 2j + 1 = \frac{1}{(n-1)^2}$$

이것을 아까의 `k`식에 대입해 보겠다.

$$\to k = 2\sqrt{Nn/(n-1)} - \frac{1}{(n-1)^2}$$
