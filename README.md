...
수소원자의 슈뢰딩거 방정식의 E는 방출되는 광자의 에너지를 비교적 잘 설명해 준다. 그러나 빛-에테르의 파동함수에서 사용된 `Es`는 실제 방출 파장을 나타내는 `Eth`와 차이가 있다. 파동함수의 `Es`는 그래프에서 보이듯 마디(`node`)의 위치를 나타내는 것에는 효과적이지만 방출되는 광자의 파장과 비교해 `1/(n-1)` 만큼 적다. 그 차이를 없애기 위해서는 '그냥 그렇다'고만 표현할 수는 없다. 그 근거를 찾아야 한다. 방법은 두 가지이다. `n-1`이 없고 `Nn`이 있는 값을 찾거나 그 반대를 찾으면 된다. 물론 이는 근거를 가지고 있어야 한다.

#### 3-2-1. 마디(`node`)의 위치

먼저 마디(`node`)의 위치를 찾아보자. `Rth`는 꺾이는 지점이 하나가 있는 함수이기 때문에 미분해서 `0`이 되는 값을 찾으면 된다. `r`을 제외한 항을 `a`로 치환하고 전체를 미분한다.
