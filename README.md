...
그래프를 다시 그려보면, 위치는 조금씩 보정이 일어났고, `l`에 대한 보정도 일어난 것을 볼 수 있다. 나는 여기서 이것이 정확한 방정식이라고 주장하는 것이 아니라 수소원자의 슈뢰딩거 방정식과 비교하여 빛-에테르 방정식의 `l`은 다른 의미를 가진다는 것을 주장하는 것이다. `l`은 에테르의 에너지와 관계되는 `Nth`와 위치와 관계되는 `nth`를 가진 값이다.
