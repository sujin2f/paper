...
<p align="center">식3-6. Nth 재정의</p>

두번째 `Nth`의 두 개의 인자는 첫번째 인자에서 두번째 인자 까지의 빛-에테르를 지칭한다. 두번째 빛-에테르는 `Nth(2)`, 두번째 에서 다섯번째 빛-에테르는 `Nth(2,5)`로 표기할 수 있다.

### 3-3. `Rth`의 보정, 부양자수 `l`

`Rth(r)`은 수소 원자 오비탈의 마디(`node`)와 대략적으로 일치하지만 (1) 오차가 있으며 (2) 부양자수 `l`을 포함하지 못한다. 여기서는 이 두 가지 문제를 해결해 보도록 노력해 보겠다. 먼저 밝히자면 여기서의 보정은 약간 억지스러운 숫자 놀음이다. 그러나 이 보정으로 우리는 `l`이 특별한 수임을 알 수 있게될 것이다.
...
$$R_{th}(r, n_{th}) = Ae^{-\frac{x}{2}} x^{\frac{k-1}{2}} L_{n-l-1}^{k}(x)$$
...
이제 `L`항을 보도록 하자. 여기서 아래의 인자는 `n - l - 1`이며 이 값을 항상 `1`인 것으로 간주했다. `Rth`의 유도 과정을 보면 이는 `j`값이다.
