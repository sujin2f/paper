...
이 형태는 식2-5와 일치한다. 먼저 괄호항의 세번째 항(\frac{l(l+1)}{x^2})이 식2-5의 $\frac{k^2-1}{4x^2}$과 일치하는 것으로($l(l+1) = k^2-1$) l과 k의 관계를 구할 수 있다. ($k=2l+1$) 또한 괄호항의 두번째 항($\frac{2 \mu}{ℏ^2\epsilon^2}V$)이 $\frac{2j+k+1}{2x}$와 일치하는 것을 가정해 E와 V의 관계를 얻을 수 있다. 여기서 $2j+k+1$는 $2j+2l+2$로 치환되며, 이를 2로 나눈 값 $2j+2l+2$는 주양자수 n으로 변환된다. 그러나, 빛-에테르는 n=2부터 시작하기 때문에 이를 n-1로 변환하기로 하겠다.

$$\frac{2 \mu}{ℏ^2\epsilon^2}V = \frac{2j+k+1}{2x}$$

$$\to \frac{2 \mu}{ℏ^2\epsilon^2}V = \frac{n-1}{x}$$

$$\to \epsilon^2 = \frac{2 \mu x}{ℏ^2(n-1)}V$$
...
이제 x를 대입해 보기로 하자. 그러나 잠시 식2-3의 에너지가 E 값인지 생각해 보기로 하자. 식2-3은 전자가 가진 에너지로 생각해온 에너지를 변환한 것이다. 빛-에테르는 질량의 형태로 저장되어 있다가 식2-3의 에너지를 방출하게 된다. 즉, 이것은 E가 아니라 빛-에테르가 가진 퍼텐셜 에너지로 보는 것이 합당할 것이다. 그를 바탕으로 E와 x를 구해보자.

$$E = -\frac{x}{4(n-1)}\frac{ℏ^2}{2\mu a_0^2} (\frac{1}{(n-1)^2} - \frac{1}{n^2})$$

$$\to E = -\frac{ℏ^2x}{8 \mu a_0^2 (n-1)} (\frac{1}{(n-1)^2} - \frac{1}{n^2})$$

$$\to x = \sqrt{\frac{8\mu r^2}{ℏ^2}\frac{ℏ^2x}{8 \mu a_0^2 (n-1)} (\frac{1}{(n-1)^2} - \frac{1}{n^2})}$$

$$\to x = \sqrt{\frac{ r^2}{2}\frac{x}{a_0^2 (n-1)} (\frac{1}{(n-1)^2} - \frac{1}{n^2})}$$
