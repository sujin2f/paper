...
우리가 구하려는 것은 n당 오비탈의 마디 한 개이다. 따라서 라게르 다항식의 밑은 언제나 1인 것으로 가정하자. 또한 우리는 이 단계에서 l=0인 경우만 생각할 것이다. 식2-10에서 x와 L항이 제거된 최종적인 형태의 파동함수는 다음과 같다.

$$R_n(r) = Ae^{-x/2}$$

$$R_n(r) = Ae^{-\frac{r}{a_0} \sqrt{(\frac{1}{(n-1)^2} - \frac{1}{n^2}) \frac{1}{n-1}}}$$

<p align="center">식2-11. 지름 파동함수 R(r), n > 1, l = 0</p>
...
이렇게 구해진 파동함수는 슈뢰딩거 방정식을 만족하는지 검산이 필요하다. 이를 위해 V(r) 값을 보다 정확히 구해보기로 하자. 식2-9로 부터 시작한다.
