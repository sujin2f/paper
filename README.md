...
잘 일치하지만 이건 매우 당연한 결과이다. 고전역학의 뤼드베리 공식이 수소원자에서 잘 맞는다는 것을 보여주는 것이기 때문이다. 수소에서는 에테르가 원형인지 선형인지와는 상관없이 값이 잘 맞는다. 여기서는 에테르의 표기법과 에너지 계수라는 개념을 가져가기로 하고, 방출 파장이 에테르를 대변하는 에너지 계수라는 것만 인식하도록 하자. 즉, 첫번째 항에서의 방출 파장은 `Nth(4, 1)`에 의해 결정되었으며, 이는 `1lth`, `2nth`, `3nth`, `4nth`가 가진 에너지의 합과도 통한다.
