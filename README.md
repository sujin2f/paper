...
특정 파장의 광자를 흡수한 전자는 그 궤도가 n이라는 정수만큼 증가하며, 해당 파장의 광자를 방출할 때 궤도는 감소한다. 여기서 에테르 독립적 스파클인 광자가 질량을 가진 스파클 독립적 에테르로 변화하여 궤도 사이에 들어가는 것으로 생각의 틀을 바꿔보도록 하자.
...
$$\dfrac{1}{\lambda} = R(\dfrac{1}{1^2} - \dfrac{1}{n^2}) \hspace{20pt} \\\{ R=1.0974 \times 10^7 m^2 \\\}$$
...
### 2-2. 빛 에테르의 에너지
빛 에테르가 일정한 파장을 가지고 있다면 그 에너지를 구할 수 있을 것이다. 빛의 에너지는 `E=hc/λ`의 식으로 구할 수 있다. 최종적으로 eV를 단위로 갖는 에너지 값은 아래의 수식과 같으며, 이를 정리하면 아래의 표와 같다. 여기서 `1/(n-1)^2 - 1/n^2`을 함수 Nn이라 하자.
...
여기서 `1/n^2`을 `Nn`으로 치환하고, J를 eV로 바꾸면 표2-6과 거의 일치하는 것을 알 수 있다. 여기서 에너지는 양수로 표기하기로 하고 이를 `Eth`라 명한다.

$$E_{th} = \frac{ℏ^2}{2 \mu a_0 ^ 2}N_n \hspace{20pt} \\\{ n \geqq 2 \\\}$$
...
가설에 따르면 전자는 그저 존재할 뿐이며 빛-에테르는 전자와 원자핵 사이의 공간에 들어오는 것이다. 오비탈에서 검은 부분을 마디(`node`)라 하는데, 이 부분이 빛의 에테르일 것이다. 주 양자수 `n`에 따라 원형 에테르의 개수가 증가하고 부양자수 `l`에 따라 원형의 에테르 하나가 선형으로 변화하는 것으로 생각해볼 수 있다. 빛 에테르 분포가 전자 오비탈의 마디와 일치한다면 오비탈의 마디는 사실 빛 에테르라는 것을 검증할 수 있을 것이다. 마디의 위치는 슈뢰딩거 방정식의 지름 파동함수 `R(r)` 방정식으로 알아볼 수 있다.
...
이제 $(\frac{\epsilon}{2})^2 = -\frac{2\mu}{ℏ^2}E$로, 그리고 $x=r\epsilon$로 치환하여 위 식을 아래와 같이 만든다.

$$(\frac{\epsilon}{2})^2 = -\frac{2\mu}{ℏ^2}E \hspace{10pt} $x=r\epsilon$$
...
<p align="center">식2-7. E와 n의 관계식</p>

여기서 $\alpha$가 식2-3이라고 가정해 보자. 그렇다면 최종적으로 얻는 에너지는 아래와 같으며 이를 $E_n$이라 하겠다.

$$\to E_n = -\frac{1}{n-1} E_0$$

<p align="center">식2-8. $E_n$</p>
...
이 x값을 식3-2에 대입하는 것으로 지름 파동함수의 해를 구할 수 있다. 먼저 y(x)를 다시 rR(r)로 되돌린다. 아래에서 A는 정규화 상수이다. 0에서 무한대의 적분값을 1로 놓아 얻을 수 있다.
...
우리가 구하려는 것은 n당 오비탈의 마디 한 개이다. 따라서 라게르 다항식의 밑은 언제나 1인 것으로 가정하자. 또한 우리는 이 단계에서 l=0인 경우만 생각할 것이다. 위 식에서 x와 L항이 제거된 최종적인 형태의 파동함수는 다음과 같다.

$$R_n(r) = Ae^{-x/2}$$

$$R_n(r) = Ae^{-\frac{r}{a_0} \sqrt{(\frac{1}{(n-1)^2} - \frac{1}{n^2}) \frac{1}{n-1}}}$$

<p align="center">식2-9. 지름 파동함수 R(r), n > 1, l = 0</p>
...
이렇게 구해진 파동함수는 슈뢰딩거 방정식을 만족하는지 검산이 필요하다. 이를 위해 V(r) 값을 보다 정확히 구해보기로 하자. 식2-7로 부터 시작한다.

$$\frac{x}{2}V(r) = -E_0$$

x도 $E_0$로 표현하자.

$$x = \frac{2r}{a_0} \sqrt{ (\frac{1}{(n-1)^2} - \frac{1}{n^2}) \frac{1}{n-1}}$$
