...
이제 `L`항을 보도록 하자. 여기서 아래의 인자는 `j`이며 이 값을 항상 `1`인 것으로 간주했다. 우리는 앞에서 `(2j + k + 1)/2`를 `n - 1`으로 치환했다. 그런데 다시 수소원자로 돌아가보면, 이 과정에서의 `n - 1`은 원래 `n`이었으며, 이는 `x`에서 `n^2`으로 위치하게 된다. 앞서 밝힌대로 숫자 놀음을 해보기로 하자. 위에서 `n - 1`이 `n - l - 1`로 변화한 것 까지를 포함해 `(2j + k + 1)/2`의 제곱을 `n - l - 1`으로 치환하는 것이다.

$$\frac{2j + k + 1}{2}^2 = n-l-1$$

$$\to \frac{2j + k + 1}{2} = \sqrt{n-l-1}$$

$$\to 2j + k + 1 = 2\sqrt{n-l-1}$$

$$\to 2j + k = 2\sqrt{n-l-1} - 1$$
...
$$\frac{2\sqrt{n-l-1} - 3 - 1}{2}$$

$$\frac{2\sqrt{n-l-1} - 4}{2}$$
