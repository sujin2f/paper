...
$$R_{th}(r, n_{th}) = Ae^{-\frac{r}{a_0} \sqrt{N_{th}(n_{th}) \frac{1}{n_{th}}}} (\frac{2r}{a_0} \sqrt{N_{th}(n_{th}) \frac{1}{n_{th}}})^l L_{n-l-1=1}^{k}(x) \hspace{10pt} \\\{ n_{th} \geqq 1 \\\}$$

부양자수 `l`이 증가한다는 것은 원형의 빛-에테르가 선형으로 변화하는 것을 뜻하며, `l`이 적용된 수소원자의 슈뢰딩거 방정식에서는 마디(`node`)가 하나 줄어드는 것을 볼 수 있다. 따라서 에테르의 위치를 뜻하는 `1/nth` 부분은 `1/(nth-l)`로 고쳐쓰는 것이 타당해 보인다.

$$R_{th}(r, n_{th}) = Ae^{-\frac{r}{a_0} \sqrt{N_{th}(n_{th}) \frac{1}{n_{th}-l}}} (\frac{2r}{a_0} \sqrt{N_{th}(n_{th}) \frac{1}{n_{th}-l}})^l L_{n-l-1=1}^{k}(x)$$
