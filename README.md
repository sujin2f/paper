...
수소원자의 슈뢰딩거 방정식의 E는 방출되는 광자의 에너지를 비교적 잘 설명해 준다. 그러나 빛-에테르의 파동함수에서 사용된 `Es`는 실제 방출 파장을 나타내는 `Eth`와 차이가 있다. 파동함수의 `Es`는 그래프에서 보이듯 마디(`node`)의 위치를 나타내는 것에는 효과적이지만 방출되는 광자의 파장과 비교해 `1/(n-1)` 만큼의 차이를 보인다. 이것을 다시 말하자면 `Es`에만 존재하는 `1/(n-1)`은 에테르의 위치를, 모두에게 존재하는 `Nn`은 에테르가 가진 에너지의 크기를 대변한다 할 수 있다. 에테르는 질량이기 때문에 이것은 에테르의 질량으로도 말할 수 있을 것이다. `n-1`은 `n`번째 에테르를 나타내는데 효과적인 방법이 아니다. 따라서 `nth = n-1`으로 새로 정의하고 `Nn`을 다음과 같이 정의하겠다.
...
두번째 `Nth`의 두 개의 인자는 첫번째 인자에서 두번째 인자 까지의 에테르를 지칭한다. 두번째 에테르는 `Nth(2)`, 두번째 에서 다섯번째 에테르는 `Nth(2,5)`로 표기할 수 있다.
...
우리는 `L`항의 `n-l-1` 부분이 항상 `1`이 되는 것으로 가정해서 제거 했다. 따라서 오차의 보정은 `x^l`항에서 일어나게 될 것이다. 그렇다면 자연스럽게 `l`이 의미하는 바가 새롭게 정의될 것이다. 위에서 정의한 `Nn`까지 적용해서 `Rth`를 다시 써보기로 하겠다.

$$R_{th}(r, n_{th}) = Ae^{-\frac{r}{a_0} \sqrt{N_{th}(n_{th}) \frac{1}{n_{th}}}} (\frac{2r}{a_0} \sqrt{N_{th}(n_{th}) \frac{1}{n_{th}}})^l L_{1}^{k}(x) \hspace{10pt} \\\{ n_{th} \geqq 1, l = 0 \\\}$$
