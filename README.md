...
이것을 빛-에테르로 표기해 보겠다. 에테르는 원형과 선형이 존재한다. 원형은 에테르의 위치 번호와 `n`으로, 선형은 위치 번호와 `l`로 표기하겠다. 이 표기법에 따라 `3s`는 원형 에테르 두 개를 의미하기에 `[1n, 2n]`으로 표시할 수 있다. 선형 에테르 하나와 원형 에테르 하나로 이루어진 `2p`의 표기법은 `[1l, 2n]`으로 하겠다. 방출 파장은 뤼드베르 공식을 따르겠다.

$$\lambda = \frac{1}{R(N_{th})}$$
