...
### 빛 에테르의 에너지
빛 에테르가 일정한 파장을 가지고 있다면 그 에너지를 구할 수 있을 것이다. 빛의 에너지는 $E=h\dfrac{c}{\lambda}$의 식으로 구할 수 있다. 최종적으로 eV를 단위로 갖는 에너지 값은 $E=Rhc(\dfrac{1}{(n-1)^2} - \dfrac{1}{n^2})*6.242*10^32$이며, 이를 정리하면 아래와 같다.
