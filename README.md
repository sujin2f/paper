...
수소원자의 슈뢰딩거 방정식의 E는 방출되는 광자의 에너지를 비교적 잘 설명해 준다. 그러나 빛-에테르의 파동함수에서 사용된 `Es`는 실제 방출 파장을 나타내는 `Eth`와 차이가 있다. 파동함수의 `Es`는 그래프에서 보이듯 마디(`node`)의 위치를 나타내는 것에는 효과적이지만 방출되는 광자의 파장과 비교해 `1/(n-1)` 만큼 적다. 이는 빛-에테르는 스파클이 존재하지 않는 순수한 질량이며, 그 질량 자체가 광자로 전환되어 방출이 되는 것이라 다른 접근법이 필요하다고 생각해볼 수 있다. 여기서는 파동함수의 `Es`에 의해 결정되는 마디(`node`)의 위치가 그 에너지를 결정하는 것으로 가정하고 그 관계를 찾아보도록 하겠다. 즉, `Es`와 `Eth`는 연관이 있다는 것이다.

먼저 마디(`node`)의 위치를 찾아보자.
