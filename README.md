...
### 2-2. 빛 에테르의 에너지
빛 에테르가 일정한 파장을 가지고 있다면 그 에너지를 구할 수 있을 것이다. 빛의 에너지는 `E=hc/λ`의 식으로 구할 수 있다. 최종적으로 eV를 단위로 갖는 에너지 값은 아래의 수식과 같으며, 이를 정리하면 아래의 표와 같다.
...
<p align="center">식2-2. 수소 원자에서의 각 준위(n)에 따른 전자의 에너지</p>

여기서 $\frac{1}{n^2}$을 $\frac{1}{(n-1)^2} - \frac{1}{n^2}$으로 치환하고, J를 eV로 바꾸면 표2-6과 거의 일치하는 것을 알 수 있다. 여기서 에너지는 양수로 표기하기로 하고 이를 $E_0$라 명한다.

$$E_0 = \frac{ℏ^2}{2 \mu a_0 ^ 2} (\frac{1}{(n-1)^2} - \frac{1}{n^2})$$

<p align="center">식2-3. 준위차에 따른 빛-에테르 에너지 가설 $n \geqq 2$</p>
...
### 2-3. 슈뢰딩거 방정식과 오비탈로 보는 빛 에테르 (n > 1, l = 0)
