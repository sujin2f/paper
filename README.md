...
$$R_{th}(r) = Ae^{-\frac{r}{a_0} \sqrt{(\frac{1}{(n-1)^2} - \frac{1}{n^2}) \frac{1}{n-1}}} \hspace{10pt} , \hspace{10pt} A = (\frac{1}{2a_0} \sqrt{(\frac{1}{(n-1)^2} - \frac{1}{n^2}) \frac{1}{n-1}})^{\frac{3}{2}} \\\{ n \geqq 2, l = 0 \\\}$$
...
이렇게 구해진 파동함수는 슈뢰딩거 방정식을 만족하는지 검산이 필요하다. 이를 위해 V(r) 값을 보다 정확히 구해보기로 하자. 식3-4로 부터 시작한다.
