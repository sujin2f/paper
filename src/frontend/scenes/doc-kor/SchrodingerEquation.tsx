...
export const SchrodingerEquation = (): JSX.Element => (
    <Doc>
        ...
        <p>
            주의: 본 문서는 엄밀한 수학적 과정을 담지 못하고 있다. 값을 임으로
            조작한 결과이기 때문이다. 부끄러우니까 안 읽으셔도 좋다.
        ...
        <p>
            ...
            그렇다면 전자의 슈뢰딩거 방정식을 변화시켜서 광자-에테르 방정식을
            만들 수 있지 않을까? 원형 에테르인 마디의 위치는 슈뢰딩거 방정식의
            지름 파동함수 R(r) 방정식으로 알아볼 수 있다.
        </p>
        ...
        <p>
            수소 원자의 지름 파동함수의 해를 구하는 방법 중 변형된 버금 라게르
            함수꼴로 만드는 방법을 사용할 것이다.
        </p>
        ...
        <p>
            이 형태는 버금 라게르 함수와 일치한다. 먼저 l을 포함하는 대괄호 항의
            세 번째 항이 버금 라게르 함수의 k<sup>2</sup> - 1을 포함하는 항과
            일치하는 것으로{' '}
            <Latex>{`$l (l + 1) = (k^2 - 1) / 4 \\to k = 2l + 1$`}</Latex>, l과
            k의 관계를 구할 수 있다. k = 2l + 1. 또한 위 식의 V(r)을 포함하는
            항이 <Latex>{`$\\dfrac{2j + k + 1}{2x}$`}</Latex>와 일치하는 것을
            가정해 E를 다시 구하고자 한다. 여기서{' '}
            <Latex>{`$\\dfrac{2j + k + 1}{2}$`}</Latex>는 j + l + 1로 볼 수
            있으며, 이는 수소 원자의 슈뢰딩거 방정식 풀이에서 주양자수 n으로
            변환된다. 그러나, 광자-에테르는 n이 1보다 큰 경우만 다루기 때문에
            이를 n - 1로 변환하기로 하겠다.
        </p>
        ...
        <p>
            우리는 이미 에너지를 알고 있다. E<sub>th</sub>(n)가 그것이다. 그러나
            퍼텐셜 에너지 V(r)에 대해서는 아는 바가 없다. 여기가 너무
        ...
        <p>
            이 <Latex>{`$E\\alpha$`}</Latex>를 다시 치환 조건에 대입해 x 값을
            얻어낸다.
        </p>
        ...
        <p>
            ...
            다항식의 밑은 언제나 1인 것으로 가정하자. 또한 우리는 이 단계에서 l
            = 0인 경우만 생각할 것이다. 위 식에서 x와 L 항이 제거된 최종적인
            형태의 파동함수는 다음과 같다.
        </p>
        ...
        <p>
            여기서 기대하는 결과는 기존의 오비탈의 마디와 광자-에테르의 고점이
            일치하는 것이다. 그렇다면 가설로 세운 전자와 원자핵 사이에
            광자-에테르가 들어가는 것이 성립하게 된다. 그 결과는{' '}
            <a
        ...
        <p>
            R<sub>th</sub>(r)은 수소 원자 오비탈의 마디와 대략 일치하지만 (1)
            오차가 있으며 (2) 수소 파동함수에서 쓰이는 부양자수 l을 포함하지
            못한다. 여기서는 이 두 가지 문제를 해결해 보도록 노력해 보겠다. 먼저
            밝히자면 아까보다 훨씬 억지스럽다. 그러나 이것으로 수소 원자에서 n
            <sup>2</sup>이 <Latex>{`$N_{th}(n)\\dfrac{1}{n-1}$`}</Latex>로
            변화한 것과 같이 l에 대한 보정이 필요하다는 것을 말하고자 한다.
        </p>
        ...
        <p>
            우리는 L 항의 j 부분이 항상 1이 되는 것으로 가정해서 제거했다. 이
            j가 항상 1이 되기 위해서는 l 값을 재정의해야한다. 그를 x<sup>l</sup>{' '}
            항에서의 l 값이 부양자수가 아닌 것이 될 것이다. R<sub>th</sub>(r)를
            다시 보자.{' '}
        </p>
        ...
        <p>
            오비탈을 다시 보자. l이 증가함에 따라 원형의 마디(node)가 줄어들고
            선형의 마디(node)가 생겨난다. 부양자수 l이 증가한다는 것은 원형의
            광자-에테르가 선형으로 변화하는 것을 뜻하며, l이 적용된 수소 원자의
            슈뢰딩거 방정식에서는 마디(node)가 하나 줄어드는 것을 볼 수 있다.
            따라서 에테르의 위치를 뜻하는 <Latex>{`$\\dfrac{1}{n-1}$`}</Latex>{' '}
            부분은 <Latex>{`$\\dfrac{1}{n-l-1}$`}</Latex>로 고치는 것이 타당해
            보인다.
        </p>
        ...
        <p>
            우리는 앞에서 <Latex>{`$\\dfrac{2j + k + 1}{2}$`}</Latex>를 n - 1로
            치환했다. 그런데 다시 수소 원자로 돌아가 보면, 이 과정에서의 n - 1은
            원래 n<sup>2</sup>이었다.
        </p>

        <p>
            또한 수소 원자에서 주양자수 n은{' '}
            <Latex>{`$\\dfrac{N_{th}(n)}{n-1}$`}</Latex>로도 전환된 것을 볼 수
            있다. 하나는 수소 원자에서 왔고 다른 하나는 내가 만든 수식에서
            왔지만 이를 합쳐보기로 하자. 말도 안 되는 일을 벌이는 거다. 왼쪽
            항은 수소 원자에서, 오른쪽은 위에서 만든 에테르에서 온 것이다.
            이렇게 l 값을 구해보도록 한다.
        </p>
        ...
        <p>
            ...
            <Latex>{`$\\dfrac{2j + k + 1}{2}$`}</Latex>은 n이 되어 x의 일부로{' '}
            <Latex>{`$\\dfrac{1}{n^2}$`}</Latex>이 된다. 역시 말도 안 되지만
            그것으로부터 출발하겠다. 우리는 n 말고 n - 1을 사용한다. k는 L의 위
            인자이기도 하다. L의 아래 인자를 1로 가정한 것과 같이 여기서도 k를
            항상 1이라 하자. 그래야 L이 항상 1이 될 수 있다.
        </p>
        ...
        <p>
            ...
            광자-에테르의 고점의 위치이다. 둘이 더욱 근사하는 것을 볼 수 있다.
        </p>
        ...
        <p>
            ...
            비교하여 x 항의 지수로 쓰이는 광자-에테르 방정식의 l은 다른 의미를
            가진다는 것을 주장하는 것이다.
        </p>
    </Doc>
)
