...
export const ClassicPhysics = (): JSX.Element => (
    <Doc>
        ...
        <p>
            ...
            로 준위가 떨어질 때 방출되는 빛의 파장인 라이먼 계열과{' '}
        ...
        <p>
            ...
            4에서 3으로 가는 수치를 라이먼 계열의 값만으로는 유추하기 힘들다는
            것이다. 우리는 n 껍질 사이의 공간이 일정한 수치가 되는 것을 원하기
            때문에 파장은 적절하지 않다. 라이먼 계열의 파장을 그 역수인 파수로
            바꾸어 표현하면 아래와 같다.
        </p>
        ...
        <p>
            ...
            있다. 여기서 λ는 앞서 구한 파수를 의미한다. 여기서 λ는 앞서 구한
            파수를 의미하며, 이렇게 구한 값은 J의 단위를 가진다. 이를 eV를
            단위로 갖는 에너지 값은 아래의 수식과 같으며, 이를 정리하면 아래의
            표와 같다. 여기서{' '}
            <Latex>{`$\\dfrac{1}{n - 1}^2 - \\dfrac{1}{n}^2$`}</Latex>를 함수{' '}
            <Latex>{`$N_{th}(n)$`}</Latex>이라 하자. 이 함수는 앞으로도 계속
            쓰일 것이다.
        </p>
        ...
        <p>
            ...
            <Latex>{`$N_{th}(n)$`}</Latex>로 치환하고, J를 eV로 변환하면 위에서
            구한 방식과 거의 일치하는 것을 알 수 있다.
        </p>
        ...
        <p>
            ...
            에너지의 값이 변하게 된다. 이 부분을 단위로 <strong>Rydberg</strong>
            라고 부른다.
        </p>
        ...
        <ul>
            ...
            <li>
                둘째, 에테르가 가진 에너지는 <strong>덧셈</strong>에 의해 구할
                수 있어야 한다.
            </li>
            <li>
