...
export const SchrodingerEquation = (): JSX.Element => (
    <Doc>
        ...
        <p>
            Caution: This chapter is not perfectly mathematical. The formula is
            somewhat manipulated. So, you can pass it to
            <button onClick={() => scrollTo('#conclusion')} className="anchor">
                the next chapter
            </button>
            .
        </p>
        ...
        <p>
            As we observed in the previous chapter, an ether creates an orbital.
            Therefore, is it possible to make a photon-ether equation to
            manipulate the hydrogen Schrödinger equation? The position of nodes
            can get with the R(r) equation, which is the radial equation.
        </p>
        ...
        <p className="align__center">
            <strong>The Radial Equation, R(r)</strong>
        </p>
        ...
        <p>Slightly Different Associated Laguerre Equation and its Solution</p>
        ...
        <p>
            ...
            First, using the third group in the bracket is the same with k
            <sup>2</sup> - 1 group in the associated Laguerre equation, we can
            get the relationship between l and k,{' '}
            <Latex>{`$l (l + 1) = (k^2 - 1) / 4 \\to k = 2l + 1$`}</Latex>.
            Also, the group includes V(r) is the same with{' '}
            <Latex>{`$\\dfrac{2j + k + 1}{2x}$`}</Latex>, so I will get what is
            E. We can say <Latex>{`$\\dfrac{2j + k + 1}{2}$`}</Latex> is j + l +
            1, and this is converted to n in the hydrogen atom. However, our
            photon-ether starts from 2, so I will set this as n - 1.
        </p>
        ...
        <p>
            We already know the energy, E<sub>th</sub>(n). However, we do not
            know about V(r). I know this is not logical enough, but let me
            assume <Latex>{`$\\dfrac{x}{2}V(r)$`}</Latex> to{' '}
            <Latex>{`$E_{th}(n)\\dfrac{1}{\\alpha}$`}</Latex>.
        </p>
        ...
        <p>
            Getting x from substitution of <Latex>{`$E\\alpha$`}</Latex>.
        </p>
        ...
        <p>
            ...
            change the form of the associated Laguerre equation. First, let me
            convert y(x) back to rR(r).
        </p>
        ...
        <p>
            What I want to get is one node per n. Therefore, let me assume n-l-1
            is always 1. We also consider only the case of l as zero. The result
            is:
        </p>
        ...
        <p>
            What I expect is that the node of the hydrogen atom radial equation
            with the peak point of the photon-ether equation. If so, it would
            tend to equal the node is the ether. The result can be seen{' '}
            <a
        ...
        <p>
            The red dotted line is the radial equation of the hydrogen atom, and
            the green one is the peak point of the photon-ether.
        </p>

        <div className="align__center">
            <img src={pic6} alt="The Photon-Ether Position" />
        </div>
        ...
        <p>
            R<sub>th</sub>(r) is close to the node of hydrogen orbital, but it
            has an approximation error, and it does not contain the l value.
            Here I will try to fix those problems. To be honest, this process
            will be more mathematically incorrect than the previous equation.
            However, this process will provide we need to adjust l value like I
            adjusted n<sup>2</sup> to{' '}
            <Latex>{`$N_{th}(n)\\dfrac{1}{n-1}$`}</Latex> above.
        </p>
        ...
        <p>
            I eliminated L to assume the j value is always 1. To make j to 1, we
            need to redeclare l. The l would not be the angular momentum. Let me
            examine R<sub>th</sub>(r) again.
        </p>
        ...
        <p>
            Please watch the orbital again. Along with the increment of l, the
            count of radial nodes decreased, and linear nodes increased. The
            increment of angular momentum means the radial ether becomes linear,
            and it is decreasing node from the hydrogen R equation. Therefore,{' '}
            <Latex>{`$\\dfrac{1}{n-1}$`}</Latex>, which means the position of
            ether, should become <Latex>{`$\\dfrac{1}{n-l-1}$`}</Latex>.
        </p>
        ...
        <p>
            We replaced <Latex>{`$\\dfrac{2j + k + 1}{2}$`}</Latex> to n - 1,
            yet for the hydrogen equation, that was n<sup>2</sup>.
        </p>

        <p>
            Also, the electron's energy n converted to{' '}
            <Latex>{`$\\dfrac{N_{th}(n)}{n-1}$`}</Latex> in the hydrogen atom.
            One comes from the hydrogen, and the other one comes from my
            equation, and I will merge them in one place. I know it is nonsense.
            The left equation is from hydrogen, and the right is from ether to
            get a new l.
        </p>
        ...
        <p>
            I will redeclare j. When we get the hydrogen equation,{' '}
            <Latex>{`$\\dfrac{2j + k + 1}{2}$`}</Latex> becomes n, and it goes
            to the part of x, <Latex>{`$\\dfrac{1}{n^2}$`}</Latex>. I will use n
            - 1 instead of n. k is also included in L. Likewise I assumed the
            bottom of L to 1, let me assume k is always 1, so L can be always 1.
        </p>
        ...
        <p>In this case, the normalizing constant is out of my capability :(</p>
        ...
        <p>
            The red dotted line is the R equation of the hydrogen atom, and the
            purple line is the adjusted photon-ether's peak point.
        </p>
