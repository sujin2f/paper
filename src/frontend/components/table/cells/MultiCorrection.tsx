...
import { getMultiCorrection } from 'src/utils/model'
...
export const MultiCorrection = (props: Props): JSX.Element => {
    ...
    return (
        <tr className="border__bottom">
            ...
            {cols.map((_, index) => {
                const correction = getMultiCorrection(
                    row.items[index + start],
                    shift,
                )
                return (
                    <td
                        key={`${row.label}-multi-correction-${index}`}
