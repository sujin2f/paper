...
export const getPercentPoint = (item: Nullable<RawData>, shift: number) => {
    const percent = getPercent(item, shift)
    return !isNaN(percent) ? percent - 100 : NaN
}

export const getPercent = (item: Nullable<RawData>, shift: number) =>
    item ? item.getPercent(shift) : NaN
