...
여기서 `diff`는 이전 값과의 차이를 의미하며 `%p`는 해당 위치에서의 N<sub>th</sub>(n) 값을 100%로 보았을 때 어느 정도의 차이를 보이는지를 나타낸다. NIST의 데이터는 한 번의 관측이 아닌, 여러 번의 관측 결과를 모두 망라한 것이라 그만큼 엄밀하다고 볼 수 있다.

에테르의 관점에서는 각 열에 해당하는 값은 자릿수가 늘어나면서 새로운 원형 에테르가 들어올 때 변화되는 EC 값이다. 이외에도 좌상단에서 우하단으로의 대각선 값 역시 주목해봐야 하는데, 이는 원형 에테르만을 보았을 때 같은 변화를 나타내기 때문이다. 예를 들어 p4는 [-O]에서 [-OO] 으로, 원형 에테르 하나에서 둘로 변화한 결과이며, 그 대각선에 위치한 d5는 [--O]에서 [--OO] 으로, 역시 원형 하나가 원형 둘이 되는 변화이기 때문이다. 좌하단의 빈 공간은 선형 에테르의 변화이기 때문에 [-]에서 [--]와 같은 변화를 의미한다. 이 역시 규칙적이어야 에테르 가설의 근거로 사용할 수 있다. 이 표의 좌우 방향과 대각선 방향, 그리고 선형의 변화를 모아놓은 시리즈가 일정한 규칙성을 가져야만 한다는 것을 강조하고 싶다.
...
전자가 둘 이상인 다전자 원자의 EC는 수소형 원자와 같이 일정하지 않다. 과학자들은 이것이 다른 전자의 영향이라 이야기한다. 그럼에도 에테르 가설을 증명하기 위해서는 이것이 단순한 덧셈은 아니라도 일정한 경향을 띤다는 것을 보여야 한다. 비교적 다양한 오비탈을 볼 수 있는 헬륨의 데이터를 가져오도록 하자.
...
선형 에테르가 첫 번째 자리에 위치할 때는 높은 값의 EC를, 그리고 두 번째는 마이너스 값을, 세 번째부터는 N<sub>th</sub>(n)로 수렴하는 경향을 보인다고 말할 수 있다. 선형 에테르가 쌓이는 방식을 설명할 수 있게 된 것이다.
