...
In each row set, the first value is a total Rydberg, and the diff means the difference from the previous value. %p is difference when N<sub>th</sub>(n) value is 100%. NIST data is from various observation, and the values are same, so it is extremely accurate.
